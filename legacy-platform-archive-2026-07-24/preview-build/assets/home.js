    (function () {
      ...
      function getCombinedReportRegistry() {
        const publicKeys = new Set(publicReportRegistry.map((item) => getReportIdentityKey(item)));
        const localReports = syncReportRegistry().map((item) => ({
          ...item,
          source: 'local'
        })).filter((item) => !publicKeys.has(getReportIdentityKey(item)));

        const seen = new Set();
        const merged = [];

        [...publicReportRegistry, ...localReports].forEach((item) => {
      ...
      function updateLatestReportLinks(registry = getCombinedReportRegistry()) {
        const latest = Array.isArray(registry) ? registry[0] : null;
      ...
      function renderReports() {
        ...
        reportGrid.innerHTML = `
          <div class="report-stack">
            <article class="report-latest">
              <div class="report-row-title">
                <span class="pill report-source-pill ${latest.source === 'public' ? 'is-public' : 'is-local'}">${latest.source === 'public' ? 'Опубликованный отчет' : 'Локальный отчет'}</span>
                <strong>${escapeHTML(latest.title)}</strong>
                <span>${escapeHTML(latest.meta || 'Неделя и дата не указаны')}</span>
                <div class="draft-meta">
                  <span class="meta-chip">${escapeHTML(getReportTimelineLabel(latest))}</span>
