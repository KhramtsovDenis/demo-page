...
## Актуальная предрелизная сборка

Для проверки новой архитектуры используется отдельная папка:

`C:\Users\xramc\OneDrive\Documents\New project\prerelease-server-build-2026-06-24`

Именно ее можно заливать на сервер как вторичную тестовую версию. Старый рабочий проект при этом не заменять, пока новая схема не будет полностью принята.

На текущей главной странице добавлена компактная кнопка “Новый формат”, которая ведет в эту папку. Это мягкий переезд: команда может пробовать новую версию рядом с действием “+ Создать отчет недели”, без отдельного большого блока.

Старая главная оставлена автономной: стили и JS находятся внутри `index.html`, чтобы она не зависела от корневой папки `assets`. Новая сборка открывается только по кнопке “Новый формат” и использует assets внутри своей папки.

Для быстрого исправления серверной главной собран пакет:

`C:\Users\xramc\OneDrive\Documents\New project\server-main-hotfix-2026-06-25`

Из него нужно загрузить в корень сервера только `index.html`.

Ключевые файлы предрелиза:

- `prerelease-server-build-2026-06-24\index.html` — главная страница тестовой версии;
- `prerelease-server-build-2026-06-24\assets\home.js` — логика главной и статусов отчетов;
- `prerelease-server-build-2026-06-24\assets\home.css` — стили главной;
- `prerelease-server-build-2026-06-24\weekly-reports\report.html` — единый просмотрщик отчетов;
- `prerelease-server-build-2026-06-24\weekly-reports\reports.json` — публичный реестр отчетов;
- `prerelease-server-build-2026-06-24\weekly-reports\data\week-25_2026-06-18.json` — данные недели 25;
- `prerelease-server-build-2026-06-24\assets\weekly-workboard.js` — логика недельного отчета;
- `prerelease-server-build-2026-06-24\assets\weekly-workboard.css` — стили недельного отчета.

Что важно по поведению:

- неделя 25 открывается через `weekly-reports/report.html?data=week-25_2026-06-18.json`;
- если в браузере есть локальная копия этого отчета, она загружается первой;
- если локальной копии нет, берутся JSON-данные из `data`;
- если JSON не загрузился, `report.html` имеет fallback на встроенные данные недели 25;
- статус отчета показывается только на главной странице;
- внутри отчета статус-бейдж не нужен.

Политика проверок:

- для простых правок сначала проверять файлы и JSON без запуска браузера;
- для визуальной проверки не использовать `file://` и не дергать Edge headless напрямую;
- стандартный маршрут проверки: поднять локальный сервер через `tools\serve-local-preview.ps1`, затем открыть `http://127.0.0.1:<порт>/` во встроенном браузере Codex;
- Edge headless использовать только как запасной вариант, если встроенный браузер недоступен;
- временные `.edge-*` папки в корне не оставлять;
- `demo-page-main` не менять даже “на секундочку”, потому что потом эта секундочка превращается в адскую путаницу.
