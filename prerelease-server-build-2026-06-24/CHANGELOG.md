...
## Pilot Link To Prerelease - 2026-06-25

### Added

- На текущую главную добавлена компактная кнопка “Новый формат” со ссылкой на `prerelease-server-build-2026-06-24/`.
- Кнопка размещена рядом с действием “+ Создать отчет недели”, без отдельного промо-блока.

### Fixed

- Главная страница возвращена к автономной старой схеме: стили и JS находятся внутри `index.html`, без зависимости от корневой папки `assets`.
- Кнопка “Новый формат” ведет в отдельную предрелизную папку, где уже лежат собственные assets новой сборки.
- В новой сборке локальный черновик новой недели больше не уходит в самый низ списка: опубликованный отчет имеет приоритет только над локальным дублем той же недели, а общий список сортируется по дате недели.
- При создании новой недели завершенные задачи из исходной недели больше не попадают в “Завершено на этой неделе”; они получают дату исходного отчета и уходят в архив завершенных проектов.
- При создании новой недели очищаются недельные поля рисков/фокуса CEO, чтобы старые риски не переезжали в новый отчет.

## Prerelease Report Workflow - 2026-06-25

### Added

- Собрана отдельная предрелизная серверная версия `prerelease-server-build-2026-06-24` для проверки новой архитектуры без замены старого рабочего проекта.
- В предрелизную сборку подтянуты данные недели 25 из `weekly_workboard_week-25_2026-06-18.html`.
- Для недели 25 создан JSON `weekly-reports/data/week-25_2026-06-18.json`; `reports.json` открывает его через `report.html?data=week-25_2026-06-18.json`.
- В `report.html` добавлен fallback на встроенные данные недели 25, чтобы отчет не разваливался полностью при проблеме загрузки JSON.

### Changed

- Статус отчета перенесен в логику главной страницы: в самом отчете бейдж статуса больше не показывается.
- На главной используются статусы `Опубликованный отчет`, `Есть изменения` и `Локальный черновик`.
- Локальные сохраненные изменения опубликованного отчета больше не вытесняют публичный отчет из реестра.
- Загрузка отчета теперь сначала проверяет локальную сохраненную копию, а JSON/встроенные данные использует только если локальных изменений нет.
- Блок ближайших ключевых результатов исключает завершенные задачи и показывает ближайшие активные релизные точки.
- Завершенные задачи убраны из активного dashboard и выводятся ниже направлений как результаты недели или архив завершенных проектов.
- Автостатусы направлений пересмотрены: `paused` не считается проблемой сам по себе, а текст в `Риски / Фокус CEO` считается сигналом внимания.
- `Ранняя стадия` переведена в синий визуальный статус, чтобы не путать раннюю фазу с проблемой.
- Плановый горизонт в строках направления сокращен до компактного формата месяцев: например `дек. 2026`.

### Fixed

- Исправлена потеря изменений после обновления страницы в опубликованном JSON-отчете.
- Исправлена логика `Частично в графике`, которая раньше слишком часто срабатывала из-за старой стартовой даты.
- Исправлен кейс, когда направление `Управление весом` оставалось `Частично в графике` при нормальном прогрессе ближайших задач к релизу 31 августа.

## Weekly Portfolio Compact Columns - 2026-05-29

### Changed

- В портфельной таблице недельного отчета “Сводная готовность” заменена на круговой индикатор.
- Колонка действия в портфельной таблице сжата до иконки без текстового заголовка.
- Освобождено больше ширины под текстовые колонки “Что меняем в продукте” и “Ближайший результат”.
...
- Рамка кнопки возврата усилена явно в CSS, чтобы она читалась как в старой версии и не терялась на светлом фоне.
- Для дорожных карт сайта, приложения и сердца возвращен текст основной кнопки “Добавить веху”.
