    ...
    },
    {
        ...
        const APP_VERSION = window[ROADMAP_VERSION_GLOBAL] || 'v-1779664338478';
...
document.addEventListener('wheel', (event) => {
        ...
        function exportHTML() {
            ...
            const stateScript = buildExportStateScript(milestones, `v-${Date.now()}`);
            ...
            scripts.forEach(script => {
                ...
                if (scriptText.includes(ROADMAP_STATE_GLOBAL) || scriptText.includes(ROADMAP_VERSION_GLOBAL)) {
                    script.remove();
                    return;
                }
                script.remove();
        ...
        function buildExportStateScript(data, version) {
            return `<script>window.${ROADMAP_STATE_GLOBAL}=${serializeForHtml(data)};window.${ROADMAP_VERSION_GLOBAL}=${JSON.stringify(version)};<\/script>`;
        }
