const INITIAL_DATA = Array.isArray(window.__ROADMAP_INITIAL_DATA__)
            ? window.__ROADMAP_INITIAL_DATA__
            : [];

        const APP_VERSION = String(window.__ROADMAP_APP_VERSION__ || 'v-1775266213');
        ...
        const lastViewportMode = { mobile: isMobileView() };
        ...
        function restoreDetailState() {
            const fallbackInitialDetails = buildInitialDetailData(milestones);
...
document.addEventListener('wheel', (event) => {
        ...
        function setupViewportWatcher() {
            window.addEventListener('resize', debounce(() => {
                ...
                if (currentMobile !== lastViewportMode.mobile) {
        ...
        function render() {
            ...
            container.innerHTML = '';
            container.className = `roadmap-container ${isMobileView() ? 'mobile' : 'desktop'}`;
        ...
        function handleDragEnd() {
            ...
            dragStartIndex = -1;
            document.querySelectorAll('.desktop-column').forEach(col => col.classList.remove('drag-over'));
        }
        ...
        function exportHTML() {
            ...
            scripts.forEach(script => {
                if (!['app-script', 'roadmap-state'].includes(script.id) && !(script.src || '').includes('mammoth') && !(script.src || '').includes('jszip')) {
                    script.remove();
                }
            });

            const stateScript = clone.querySelector('#roadmap-state');
            if (stateScript) {
                const stateStr = JSON.stringify(milestones, null, 4)
                    .replace(/</g, '\\u003c')
                    .replace(/>/g, '\\u003e')
                    .replace(/&/g, '\\u0026');
                stateScript.textContent = `\n        window.__ROADMAP_INITIAL_DATA__ = ${stateStr};\n        window.__ROADMAP_APP_VERSION__ = ${JSON.stringify(`v-${Date.now()}`)};\n    `;
            }
